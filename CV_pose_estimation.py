...
#### ADJUST VALUES BELOW ####
    ...
    def visualize_arms_up(self, arms_up_list, pygame_surface):
        ...
        for keypoints in arms_up_visualization:
            ...
            shoulder = tuple(float(coord) for coord in shoulder)
            elbow = tuple(float(coord) for coord in elbow)
            wrist = tuple(float(coord) for coord in wrist)

            # Validate the format of the converted keypoints
            assert isinstance(shoulder, tuple) and len(shoulder) == 2, "Shoulder must be a tuple of (x, y)"
            assert isinstance(elbow, tuple) and len(elbow) == 2, "Elbow must be a tuple of (x, y)"
            assert isinstance(wrist, tuple) and len(wrist) == 2, "Wrist must be a tuple of (x, y)"

            print(f"Converted keypoints - Shoulder: {shoulder}, Elbow: {elbow}, Wrist: {wrist}")
